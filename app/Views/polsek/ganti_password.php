...
<div class="xl:mx-72 lg:mx-64 md:mx-32 sm:mx-20 mx-10 mt-36">
    <form action="<?= base_url('/polsek/change-password') ?>" method="post" enctype="multipart/form-data"
        class="flex flex-col pb-12 bg-white dark:bg-gray-800 rounded-lg">
        <?= csrf_field() ?>
        <button type="button" class="flex justify-start dark:text-gray-200 md:ml-10 md:mt-10 ml-6 mt-6"
            onclick="history.back()"><i class="fa-solid fa-chevron-left md:text-2xl text-xl"></i></button>
        ...
        <div class="flex justify-center items-center xl:mx-48 lg:mx-28 mx-20 mt-8">
            <button type="submit"
                class="p-2 rounded-lg text-white font-bold w-full bg-gray-900 dark:border dark:border-gray-500">
                Simpan
            </button>
        </div>
    </form>
...
<script>
// alert error 
<?php if (!empty(session()->getFlashdata("error_change_password"))) : ?>
document.addEventListener("DOMContentLoaded", () => {
    Swal.fire({
        title: "error!",
        icon: "error",
        html: `<?= session('error_change_password') ?>`
    });
})
<?php endif; ?>

// alert error 
<?php if (!empty(session()->getFlashdata("success_change_password"))) : ?>
document.addEventListener("DOMContentLoaded", () => {
    Swal.fire({
        title: "success!",
        icon: "success",
        html: `<?= session('success_change_password') ?>`
    });
})
<?php endif; ?>
</script>
